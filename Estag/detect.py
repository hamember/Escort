...
def code_tracking(dst_points, im):
    ...
    tag, ori = check(code)
    return tag, ori, transform_points


def locate(im, thresh_method=1, block_size=15, c=3, thresh=None, mean_area=700, valid_tags=None, visual=0,
           config_file=None, detect_area=None):
    ...
    area_thresh = [0.1 * mean_area, 1.25 * mean_area]
    if detect_area:
        original_im = im
    ...
    for contour in contours:
        ...
        if len(box) != 4:
            continue
        tag, orientation, points_16 = code_tracking(box, bw // 255)
    ...
    if visual == 1:
        visualization(im, tags, orientations, boxes, not_pass_boxes)
...
def portion_visual(im, box, points_16_all):
    ...
    for i in range(len(points_16_all)):
        color = (0, 0, 255)
        cv2.circle(portion_im, tuple(np.int32((points_16_all[i]-np.array([x1-5, y1-5]))*5)), 5, color, thickness=-1)
    cv2.imshow('portion_bin', portion_im)
    cv2.waitKey(0)
    cv2.imwrite('portion.jpg', portion_im)


def visualization(im, tags, orientations, boxes, not_pass_boxes):
    for i, box in enumerate(boxes):
        for j, point in enumerate(box):
            color = (0, 255, 0)
            # cv2.circle(im, tuple(np.int32(point[0])), 5, color, thickness=2)
            # cv2.imwrite('points.jpg', im)
        cv2.putText(im, str(tags[i]), tuple(np.int32(point[0])), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 255, 255), 4)
    # for box in not_pass_boxes:
    #     for point in box:
    #         cv2.circle(im, tuple(np.int32(point[0])), 5, (255, 0, 0), thickness=-1)
    cv2.namedWindow('detect', cv2.WINDOW_NORMAL)
    cv2.imshow("detect", im)
    cv2.imwrite('bumblebeeImage.jpg', im)
    cv2.waitKey(0)


if __name__ == '__main__':
    file = "E:/bee/BeeTag_new/data/videos/00000000_00000000001A0011.mp4"
    video = cv2.VideoCapture(file)
    pos = video.get(cv2.CAP_PROP_FRAME_COUNT) - 1
    _, im = video.read()
    cv2.namedWindow('detect', 0)
    locate(im, block_size=41, c=5, visual=1)
